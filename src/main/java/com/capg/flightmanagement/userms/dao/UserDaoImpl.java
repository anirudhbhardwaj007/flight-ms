...
import com.capg.login.AuthenticationDaoImpl;


public class UserDaoImpl extends AuthenticationDaoImpl<BigInteger, Password> implements IUserDao {
	...
	@Override
	public Map<BigInteger, Password> store() {
		Map<BigInteger, Password>store=new HashMap<>();
		userList.forEach(user->store.put(user.getId(),user.getPassword()));
		return store;
	}
}
